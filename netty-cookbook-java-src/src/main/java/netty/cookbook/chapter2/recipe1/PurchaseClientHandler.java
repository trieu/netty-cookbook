 ...
 * the server.
 */
public class PurchaseClientHandler extends SimpleChannelInboundHandler<PurchaseData> {
	
	PurchaseData message;
	CallbackProcessor asynchCall;
	boolean close = false;

    public PurchaseClientHandler(PurchaseData message, CallbackProcessor asynchCall) {
    ...
    @Override
    public void channelRead0(ChannelHandlerContext ctx, PurchaseData msg) {
